...
public abstract class Page
{
	...
	protected StringReplace replaceWithForLoop(StringBuffer buffer, int startPoint, int endPoint, Map<String, Object> paramMap)
	{
		...
		Matcher forMatch = forPattern.matcher(buffer.substring(startPoint, endPoint));
		if (forMatch.find())
		{
			String newParam = forMatch.group(1);
			String collection = forMatch.group(2);

			System.err.println("FROM: " + buffer.substring(startPoint, endPoint));
			System.err.println("Building collection = " + collection + " and " + newParam);
			Collection<Object> objects = (Collection<Object>) resolve(collection, paramMap);
	...
	protected void doReplace(StringBuffer buffer, Map<String, Object> paramMap)
	{
		...
		while (matcher.find(matchPoint))
		{
			...
			{
				replaceStack.push(replaceWithForLoop(buffer, grpStart, grpEnd, paramMap));
			}
			else
			{
				String replaceText = resolve(buffer.substring(grpStart + 2, grpEnd - 2), paramMap).toString();
